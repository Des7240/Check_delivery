...
export async function updateOrderStatus(trackingNumber, statusData) {
  ...
  // Thay vì lưu toàn bộ lịch sử (join tất cả mốc), chỉ lấy mốc mới nhất (thường nằm ở đầu hoặc cuối mảng)
  // SPX thường hiển thị mốc mới nhất ở đầu (index 0)
  const latestStep = steps && steps.length > 0 ? steps[0] : '';

  const updatePayload = {
    lastStatus: currentStatus || '',
    lastStatusDetail: latestStep,
