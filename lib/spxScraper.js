...
export async function scrapeSpxTracking(trackingNumber) {
  ...
  try {
    ...
    const trackingData = await page.evaluate(() => {
      ...
      // Tìm danh sách các mốc hành trình, ưu tiên trong section.nss-comp-tracking
      const stepSelectors = [
        'section.nss-comp-tracking .tracking-item',
        'section.nss-comp-tracking .tracking-step',
        'section.nss-comp-tracking [class*="tracking"] li',
        'section.nss-comp-tracking [class*="item"]',
        '.tracking-item',
        '[class*="tracking"] li',
      ];

      // Loại bỏ milestone progress bar nếu bị lẫn vào
      for (const sel of stepSelectors) {
        const items = document.querySelectorAll(sel);
        if (items.length > 0) {
          items.forEach(item => {
            // Không lấy text của progress bar
            if (item.closest('.comp-tracking-milestone-progress-bar')) return;
